class MessageBoard {
    ...
    The first should be a property 
    called messages which is an empty Map, and the second is a property called id which has a value of 1. 
    ...
    */
    ...
    with a key of whatever the
    value of this.id is and a value of whatever the string is that is passed to the function. The function should return 
    the object created from 
    the class so that the method can be chained. (HINT - to implement the last part, make sure to return this).
    ...
    */
    ...
    the same key as the number passed to 
    the function. If the key is not found in the messages map, the function should return undefined.
    ...
    */
    ...
    /*
    Add a method called findMessageByValue which accepts a string and returns the message in the messages map with the same value as the string
     passed to the function. If the value is not found in the messages map, the function should return undefined.
    ...
    */
    ...
    findMessageByValue(message) {
        ...
        this.messages.forEach((elem, index) => {
            if (elem == message) msg = message;
        })
        return msg;
    }

    /*
    Add a method called removeMessage which accepts a number and removes a message in the messages map with a key of the number passed to the 
    function.
...
function uniqueValues(arr) {
    var s = new Set();
    arr.forEach((elem, i) => s.add(elem));
    return s.size;
}

/*

Write a function called hasDuplicates which accepts an array and returns true if there are duplicate values in the array, otherwise it should return
 false.
...
Write a function called countPairs which accepts an array and a number. The function should return the number of 
pairs (two numbers) that sum up to
...
//same using set might be wrong as not tested by system and developed later
function countPairs(arr, count) {
    var ans = 0;
    var set = new Map();
    arr.forEach((elem) =>
        (set.get(elem) ? set.set(elem, set.get(elem) + 1) : set.set(elem, 1)))
    arr.forEach(
        elem => ans += set.get(count - elem) ? set.get(count - elem) : 0)
    return ans;
}